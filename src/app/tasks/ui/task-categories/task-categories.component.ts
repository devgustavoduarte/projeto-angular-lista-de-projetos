...
export class TaskCategoriesComponent
  extends ItemBase<Project>
  implements AddRemoveItem, OnInit
{
    ...
    this.taskService.addItem(
      new Project("01", "Titulo - Projeto", "Descricao", [
        new Task(
          ...
          "Titulo - Tarefa",
          "Lorem ipsum dolor",
          "Fugiat, iste nobis! Cum odit deserunt libero"
        ),
      ])
    );

    taskService
      .getCurrentProjectSubject()
      .next(this.taskService.getProjects()[0]);
  }
  ngOnInit(): void {}
  ...
  onAddItem(): void {
    const auxOne = this.dialogResponse(
      environment.addProjectTitle,
      ItemType.AddProject
    );
    auxOne.pipe(take(1)).subscribe((response) => {
  ...
  onRemoveItem(itemId: string): void {
    const auxOne = this.dialogResponse(
      environment.removeProjectTitle,
      ItemType.RemoveProject
    );
